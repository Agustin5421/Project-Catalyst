...
/// <summary>
/// Small ice spike used for the boss cone ability.
/// Spawns below a target and grows upward, dealing damage on touch.
/// </summary>
public class SmallIceSpike : NetworkBehaviour {
    ...
    /// <summary>
    /// Initializes the small ice spike at a target position.
    /// </summary>
    /// <param name="targetPosition">The position where the spike should grow to</param>
    public void Init(Vector3 targetPosition) {
        if (Object.HasStateAuthority) {
            ...
            Debug.Log($"SmallIceSpike initialized - Target: {TargetPosition}, Start: {StartPosition}, Ground: {groundPosition}");
        }
    }
    ...
    public override void FixedUpdateNetwork() {
        ...
        if (elapsed >= lifetime) {
            Debug.Log("SmallIceSpike lifetime expired, despawning");
    ...
    private void OnTriggerEnter(Collider other) {
        ...
        if (player != null) {
            ...
            if (playerHealth != null) {
                ...
                Debug.Log($"SmallIceSpike hit player {other.name} for {damage} damage!");
            } else {
                Debug.LogWarning($"SmallIceSpike hit player {other.name} but no PlayerHealth component found!");
            }
        }
    }
}
